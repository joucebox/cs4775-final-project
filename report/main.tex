...
To understand whether MEA provides consistent benefits across diverse RNA families or exhibits family-dependent behavior, we performed stratified analysis by RNA family. Figure~\ref{fig:family_performance} shows F1 improvement (MEA $-$ Viterbi) for selected representative families at $\gamma=0.5$ (power weighting).

Key observations:

\begin{itemize}
    \item \textbf{Highly conserved families (e.g., RF00774 rRNA):} MEA improvements are modest (0.5\%--1.5\%), as the posterior distribution is typically concentrated around a single well-defined alignment path. In these cases, Viterbi already captures the dominant signal, and MEA provides marginal gains.
    \item \textbf{Moderately divergent families (e.g., RF00236, RF00063):} MEA achieves consistent and meaningful improvements (2\%--4\%), as posterior uncertainty creates multiple plausible alignment regions that MEA can exploit.
    \item \textbf{Highly divergent families (e.g., RF00058, RF01018):} MEA improvements are variable but occasionally dramatic (up to 5\%--8\%), reflecting the stochastic nature of posterior distributions in low-similarity regimes. At these extremes, the posterior may form multiple competing modes, and MEA's ability to integrate across the ensemble becomes most valuable.
\end{itemize}

This stratified analysis confirms that MEA is particularly valuable in moderate-to-high divergence regimes, where the posterior landscape is complex but informative. In highly conserved families, the computational overhead of MEA may not be justified unless maximum achievable accuracy is the sole criterion.
...
For most weighting schemes, the confidence interval for the mean $\Delta$F1 excludes zero at intermediate $\gamma$ values (0.3--0.7), indicating statistically significant improvements. At the extremes ($\gamma < 0.2$ or $\gamma > 0.8$), confidence intervals widen, reflecting greater variability and less consistent performance across families.

The robustness analysis further examined alignment disagreement patterns. For pairs of alignments where MEA and Viterbi differed, we quantified the fraction of disagreements that corresponded to genuinely ambiguous positions (posterior probability 0.3--0.7) versus positions where one method made a clearly incorrect choice. MEA disagreements were overwhelmingly localized to ambiguous regions (87\% of cases), supporting the hypothesis that MEA makes deliberate posterior-informed choices rather than random errors.
