...
\]
Using the forward-backward algorithm (detailed in Algorithm~\ref{alg:forward_backward} of the appendix), we compute $\alpha_M(i,j)$, the forward probability of reaching position $(i,j)$ in match state $M$ while aligning $(x_i,y_j)$, and $\beta_M(i,j)$, the backward probability of completing the alignment from position $(i,j)$ in match state $M$ (under the same indexing convention). The posterior match probability is then
\[
