...
\textbf{[Figure: F1 Score Comparison]}

\subsection{Precision-Recall Trade-offs and the Role of Gamma}

One of the definitive advantages of the MEA framework is the ability to tune the alignment characteristics via the $\gamma$ parameter. Viterbi provides a single point estimate in the precision-recall space—take it or leave it. MEA, however, offers a curve.

\textbf{[Figure: Precision, Recall, and F1 vs Gamma]} illustrates this dynamic clearly.

\begin{itemize}
    \item \textbf{MEA (Power):}
    \item \textbf{MEA (Threshold):}
    \item \textbf{MEA (LogOdds):}
    \item \textbf{MEA (ProbCons):}
    \item \textbf{Viterbi:} characteristics
\end{itemize}

\subsection{Analysis of Alignment Uncertainty: The Viterbi Fallacy}

\textbf{[Figure: RF00236 Posterior Heatmap]}

To gain a qualitative understanding of \textit{why} MEA performs better, we examined the posterior probability heatmaps for specific alignments. \textbf{[Figure: RF00236 Posterior Heatmap]} presents a striking example of the ``Viterbi Fallacy.''


\begin{itemize}
    \item \textbf{The Viterbi Path:}
    \item \textbf{The MEA Path:}
\end{itemize}
